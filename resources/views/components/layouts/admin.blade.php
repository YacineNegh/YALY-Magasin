...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-50/50 font-sans text-brand-ink antialiased">
        <div class="min-h-screen lg:flex">
            <aside class="border-b border-gray-100 bg-white lg:fixed lg:inset-y-0 lg:w-72 lg:border-b-0 lg:border-r">
                ...
                <nav class="flex gap-2 overflow-x-auto px-5 pb-5 text-sm font-medium lg:grid lg:overflow-visible mt-2">
            ...
            <div class="min-w-0 flex-1 lg:pl-72">
                <header class="border-b border-gray-100 bg-white">
                    <div class="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                        <div>
                            <p class="text-xs font-bold uppercase text-gray-400">Connecté en tant que {{ auth()->user()->name }}</p>
                            <h1 class="text-2xl font-extrabold tracking-tight mt-1">{{ $heading ?? 'Admin' }}</h1>
                        </div>
                        <form method="POST" action="{{ route('admin.logout') }}">
                            @csrf
                            <button class="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-bold text-gray-700 shadow-sm transition hover:bg-gray-50 hover:text-brand-ink">Se déconnecter</button>
                        </form>
                    </div>
                </header>
                ...
                <main class="px-4 py-8 sm:px-6 lg:px-8">
                    ...
                    @endif
                    @if ($errors->any())
                        <div class="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700 shadow-sm">{{ $errors->first() }}</div>
                    @endif
